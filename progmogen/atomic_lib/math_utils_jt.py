...
def distance_3d_point_line(data, line_params):
    ...
    d = line_params[:, 3:]
    # print("d_norm = ", d.norm())
    x = jt.cross(data-p, d, dim=1).norm(dim=1)
...
def fit_3d_line(data):
    ...
    uu, dd, vv = jt.linalg.svd(data - datamean)
    vec_d = vv[0]
    assert np.allclose(jt.norm(vec_d).item(),1.0,atol=1e-4)
    # print(datamean.shape, vec_d.shape)
    line_params = jt.cat([datamean, vec_d], 0).reshape(1,-1)
    return line_params
...
def dist_to_plane(data, plane_params):
    ...
    one_col = jt.ones(joints.shape[:-1]+(1,)).float().to(joints.device)
    joints_homo = jt.cat([joints, one_col], 3)
...
def loss_dist_to_plane(data, plane_params):
    ...
    one_col = jt.ones(data.shape[0], 1).to(data.device)
    data_homo = jt.cat([data, one_col], 1)
...
def fit_yPlane(joint_pos):
    ...
    # A=[[z_i, 1], ...]^T
    b = joint_pos[:,0:1]
    A = jt.cat([ joint_pos[:,2:3], jt.ones_like(b) ], 1)
    A_pinv = jt.inverse(jt.matmul(A.t(),A))
    # btA = th.matmul(b.t(), A)
    btA = jt.matmul(A.t(), b)
    # (c,d)
    x_res = -jt.matmul(A_pinv, btA)
    x_res = x_res.reshape(-1)
    plane_params = jt.zeros(1,4).to(x_res.device)
...
def plane_params_4d_to_point_normal_form(plane_params):
    ...
    pn = jt.zeros(1,6).to(plane_params.device)
...
def plane_point_normal_form_to_params_4d(point_normal):
    # (1,6) in (point+normal)
    # return plane_params: (1,4)
    plane_params = jt.zeros(1,4).to(point_normal.device)
    p1 = point_normal[0,0]
    p2 = point_normal[0,1]
    p3 = point_normal[0,2]
    n1 = point_normal[0,3]
...
def get_center_of_mass(joints):
    ...
    weight_list_expand = jt.array(weight_list)
...
def less_than(joints, margin):
    return nn.relu( joints-margin )


def greater_than(joints, margin):
    return nn.relu( margin-joints )
...
def operation_or(joints1, joints2):
    return jt.minimum(joints1, joints2)
