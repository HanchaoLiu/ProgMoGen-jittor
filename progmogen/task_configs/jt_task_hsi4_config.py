...
def f_loss(self, sample, sample_0, step):
    ...
    # regularization on end position.
    loss_reg = ( greater_than(keyframe(dimZ( get_joint(joints, pelvis) ), self.length-1), 5.0) +  \
                 less_than(keyframe(dimZ( get_joint(joints, pelvis) ), self.length-1), 6.0) ) / 2.0 
    loss_reg = loss_reg + less_than(keyframe(dimY( get_joint(joints, pelvis) ), self.length-1), 1.2)
