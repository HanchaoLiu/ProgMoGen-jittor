...
from diffusion.respace_v2_jt import SpacedDiffusion_v2
from .gaussian_diffusion_v2_jt import _extract_into_tensor
...
class InpaintingGaussianDiffusion(SpacedDiffusion_v2):
    ...
    def load_inv_normalization_data(self, device):
        ...
        d_mean = jt.array(d_mean, dtype = jt.float)
        d_std   = jt.array(d_std, dtype = jt.float)
    ...
    def ddim_sample_loop_progressive_opt_known_noise(
        ...
        if skip_timesteps and init_image is None:
            ...
            init_image = jt.zeros_like(img)

        indices = list(range(self.num_timesteps - skip_timesteps))[::-1]

        if init_image is not None:
            # my_t = th.ones([shape[0]], device=device, dtype=th.long) * indices[0]
            my_t = jt.ones([shape[0]], dtype=jt.long) * indices[0]
        ...
        for i in indices:
            ...
            if randomize_class and 'y' in model_kwargs:
                ...
                model_kwargs['y'] = jt.randint(low=0, high=model.num_classes,
                                               size=model_kwargs['y'].shape,
                                               device=model_kwargs['y'].device)
            # with jt.no_grad():
            if True:
    ...
    def ddim_sample_loop_opt_fn_base(
        ...
        noise_list = [jt.array(a, dtype=jt.float) for a in noise_list_npy]
        noise_init = jt.array(noise_init_npy, dtype=jt.float)
        ...
        # set loss eval
        self.loss_ret_val = np.array([0.0])
    ...
    def ddim_sample_loop_opt_fn(
        ...
        optimizer = nn.Adam([noise_init], base_lr)
        ...
        for it in range(max_it):
            ...
            del pred_res, res_list, pred_x0_list, loss, loss_eval
        ...
        self.loss_ret_val = loss_confined_space.numpy()
